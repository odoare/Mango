...
public:
    BlockTextPanel()
    {
        ...
        info.setInfo ("Block overrides",
            ...
            "s4 s8 s16 s32 s64 are shortcuts for those same fractions; t4..t64 "
            "are their triplets and d4..d64 their dotted versions, the same "
            "straight/triplet/dot grid the panel above draws from. They scale "
            "the same way mididur does: s4*2, t8/3, 2*d16.\n\n"
            ...
            "A trailing s writes an absolute time instead: dur=1.5s is 1.5 "
            "seconds outright, ignoring both the tempo and mididur.\n\n"
            "Any expression using mididur, or an absolute Ns value, is a time "
            "in seconds, so it ignores the tempo. Every other key is in the "
            "same unit as its knob.\n\n"
